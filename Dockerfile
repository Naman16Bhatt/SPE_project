...
#RUN hdfs dfs -put /app/uber_combined.csv  /
#CMD ["hdfs","dfs","-put", "uber_combined.csv", " /"]
#CMD ["hdfs","dfs","-put", "taxi_combined.csv", " /"]
#CMD ["hdfs","dfs","-ls" ]
#COPY Economics.py /   Economics.py /

COPY ./Economics.py /Economics.py
COPY ./uber_combined.csv /uber_combined.csv
COPY ./taxi_combined.csv /taxi_combined.csv
...
ENTRYPOINT service ssh start && /usr/hadoop-3.0.0/sbin/start-all.sh  &&  hdfs dfs -put /uber_combined.csv /  && hdfs dfs -put /taxi_combined.csv / && spark-submit /sparkplot-residential-analysis.py && bash 
...
#RUN hdfs dfs -put taxi_combined.csv  /		
